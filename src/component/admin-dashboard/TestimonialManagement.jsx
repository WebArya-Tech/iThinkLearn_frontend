...
import { Star, Trash2, Check, X, AlertCircle, Eye, Plus, Image as ImageIcon, Upload, Save, Edit } from 'lucide-react';
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {filteredTestimonials.map((t, idx) => {
                ...
                const isMedia = !!mediaUrl && (mediaUrl.startsWith('http') || mediaUrl.startsWith('data:'));
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-center gap-1.5 flex-wrap">
                        ...
                        <button
                          ...
                          disabled={actionLoading === id || (t.status && t.status.toUpperCase() === 'APPROVED')}
                          ...
                          title={t.status && t.status.toUpperCase() === 'APPROVED' ? 'Already Approved' : 'Approve this testimonial'}
                        >
                          <Check size={13} /> Approve
                        </button>
                        <button
                          onClick={() => handleReject(id)}
                          disabled={actionLoading === id || (t.status && t.status.toUpperCase() === 'REJECTED')}
                          ...
                          title={t.status && t.status.toUpperCase() === 'REJECTED' ? 'Already Rejected' : 'Reject this testimonial'}
                        >
                          <X size={13} /> Reject
                        </button>
                        <button
        ...
        >
          ...
          >
            ...
            <p className="text-gray-700 leading-relaxed mb-4">
              {viewModal.text || viewModal.message || viewModal.quote || viewModal.content}
            </p>

            {viewModal.type === 'video' && (
              <div className="rounded-xl overflow-hidden bg-black aspect-video mb-4">
                <video
                  src={viewModal.videoUrl || viewModal.mediaUrl || viewModal.content}
            ...
            {viewModal.type === 'image' && (
              <div className="rounded-xl overflow-hidden bg-gray-100 border border-gray-200 mb-4">
                <img 
                  src={viewModal.image || viewModal.mediaUrl || viewModal.content} 
            ...
            {viewModal.type === 'audio' && (
              <div className="bg-purple-50 p-4 rounded-xl mb-4">
                <audio src={viewModal.audioUrl || viewModal.mediaUrl || viewModal.content} controls className="w-full" />
              </div>
            )}
