            ...
            {user ? (
              <div className="relative" ref={userDropdownRef}>
                ...
                >
                  ...
                  <svg className={`w-3 h-3 transition-transform ${userDropdownOpen ? 'rotate-180' : ''}`} fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" clipRule="evenodd"/></svg>
                </button>
                {userDropdownOpen && (
                  <div className="absolute right-0 mt-2 w-44 bg-white rounded-xl shadow-2xl border border-gray-100 z-50 overflow-hidden">
                    <button
                      onClick={() => { navigate(isAdmin ? '/admin-dashboard' : '/student-dashboard'); setUserDropdownOpen(false) }}
                      className="w-full text-left px-4 py-3 text-sm text-gray-700 hover:bg-blue-50 hover:text-blue-900 font-medium transition"
                    >
                      📊 Dashboard
                    </button>
                    <button
                      onClick={() => { logout(); setUserDropdownOpen(false) }}
                      className="w-full text-left px-4 py-3 text-sm text-red-600 hover:bg-red-50 font-medium transition border-t border-gray-100"
                    >
                      🚪 Logout
                    </button>
                  </div>
      ...
      <div className={`md:hidden fixed inset-y-0 right-0 w-80 max-w-[85vw] bg-gradient-to-b from-blue-900 via-blue-800 to-indigo-900 text-white z-50 flex flex-col shadow-2xl transition-transform duration-300 ease-in-out ${mobileMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        ...
        <div className="flex-1 overflow-y-auto px-4 py-6 custom-scrollbar">
          ...
          <div className="mt-8  border-t border-white/10 space-y-4">
            ...
            <button
              onClick={() => { openDemoModal(); setMobileMenuOpen(false); }}
              className="w-full bg-gradient-to-r from-yellow-400 to-orange-500 text-blue-900 font-bold rounded-2xl px-5 py-4 flex items-center justify-center gap-2 shadow-xl active:scale-[0.98] transition-all text-base"
            >
            ...
            {user ? (
              <div className="space-y-2">
                ...
                <button
                  ...
                  className="w-full py-3 rounded-2xl font-bold text-red-300 hover:text-white bg-red-500/20 hover:bg-red-500/40 transition-all text-center text-sm border border-red-400/20 flex items-center justify-center gap-2"
                >
                  🚪 Logout
                </button>
              </div>
